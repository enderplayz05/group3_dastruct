    ...
    def show_infos():
        for var in history_data:
            ...
            if get_unique_ID == var:
                ...
                tk.Label(frame, text=get_time).grid(row=2, column=1, padx=5)

                # Date
                tk.Label(frame, text="Date", font=("Arial", 10), bg="#e0e0e0").grid(row=3, column=0, padx=5, pady=5)
                tk.Label(frame, text=date).grid(row=3, column=1, padx=5)
                ...
                for item in get_borrowed_data:
                    ...
                    def on_item_selected(event):
                        ...
                        combo_values = ["Ball", "Net", "Whistle", "Jersey"]  # replace with actual item options
